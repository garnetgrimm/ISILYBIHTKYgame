...
function submitSignup() {
	...
	if(response == "P") startGame();
	else if(response == "F") $("#status").html( "<br>Connection error<br>please try again later." );
	else if(response == "E1") $("#status").html( "<br>Username Taken." );
}
...
function submitLogin() {
	...
	else if(response == "E1") $("#status").html( "<br>User not found." );
	else if(response == "E2") $("#status").html( "<br>Username and password do not match." );
}
...
function getTarget() {
	...
	var response = submitForm('getTarget', fd);
	
	gameData.target = response;
}
